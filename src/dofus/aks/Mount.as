class dofus.aks.Mount extends dofus.aks.Handler
{
   ...
   function onMountPark(sExtraData)
   {
      ...
      while(_loc5_ < _loc3_.length)
      {
         ...
         _loc11_ = _loc6_[4];
         _loc12_ = _loc6_[5];
         _loc13_ = this.api.kernel.CharactersManager.createGuildEmblem(_loc12_);
         _loc14_ = Number(_loc6_[6]);
         _loc15_ = new dofus.datacenter.MountPark(_loc7_,_loc8_,_loc9_,undefined,_loc10_,_loc11_,_loc13_,this.api.datacenter.Map.id,_loc4_,_loc14_);
   ...
   function equipError(errorType)
   {
      ...
      {
         case "-":
            _loc3_ = this.api.lang.getText("MOUNT_ERROR_INVENTORY_NOT_EMPTY");
            break;
         case "+":
