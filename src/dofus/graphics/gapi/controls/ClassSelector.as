class dofus.graphics.gapi.controls.ClassSelector extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function update()
   {
      ...
      while(_loc3_ < this._aClips.length)
      {
         ...
         _loc4_ = (this._aClips[_loc3_]._y + this._yRay) / (this._yRay * 2) * (this._maxScale - this._minScale) + this._minScale;
         ...
         {
            _loc5_ = Math.floor((_loc2_ + this._nCurrentPosition) * 180 / Math.PI % 360);
            _loc6_ = Math.floor(360 / this._aClips.length);
