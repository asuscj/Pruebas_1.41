class dofus.managers.SpeakingItemsManager extends dofus.utils.ApiElement
{
   ...
   function triggerEvent(nEvent)
   {
      ...
      {
         _loc4_ = this.api.lang.getSpeakingItemsTrigger(nEvent)[_loc3_.mood];
         if(_loc4_)
         {
            _loc6_ = [];
            _loc7_ = 0;
            ...
            {
               _loc5_ = this.api.lang.getSpeakingItemsText(_loc4_[_loc7_]);
               if(_loc5_.l <= _loc3_.maxSkin)
               {
                  if(_loc5_.r != undefined && _loc5_.r != "")
                  {
                     _loc8_ = _loc5_.r.split(",");
                     _loc9_ = false;
                     _loc10_ = 0;
            ...
            _loc11_ = false;
            _loc13_ = 10;
            _loc14_ = this.api.lang.getConfigText("SPEAKING_ITEMS_MAX_TEXT_ID");
            while(!_loc11_ && ((_loc13_ = _loc13_ - 1) && _loc6_.length))
            {
               _loc12_ = _loc6_[Math.floor(Math.random() * _loc6_.length)];
            ...
            {
               _loc15_ = Math.floor(Math.random() * 3);
            }
            else
            {
               _loc15_ = 1;
            ...
            {
               _loc16_ = this.api.lang.getConfigText("SPEAKING_ITEMS_CHAT_PROBA");
